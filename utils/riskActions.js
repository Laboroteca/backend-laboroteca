 ...
 *   - sendUserNotice(email, resetUrl) → email directo al usuario infractor
 */
...
const SMTP_FROM  = process.env.SMTP_FROM  || 'laboroteca@laboroteca.es';
const SMTP_NAME  = process.env.SMTP_NAME  || 'Laboroteca';
...
/**
 * Envía email al usuario infractor para que cambie la contraseña
 */
...
<p>Si no has sido tú, responde a este email.</p>`;
  ...
  try {
    ...
    if (LAB_DEBUG) console.log('[riskActions] Email enviado a usuario', email, info.messageId);
    return { ok:true };
  } catch (err) {
    console.error('❌ [riskActions] Error enviando email usuario:', err?.message || err);
